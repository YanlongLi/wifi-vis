...
DeviceFeature.prototype.init = function(){
	var that = this;
	that.recordCount = that.path.length;
	that.aveAP = _.uniq(this.path, function(d){return d.apid}).length;
	that.aveFloor = _.uniq(this.path, function(d){return d.floor}).length;
	that.time_total = 0;
	that.path.forEach(function(r,i){
		if(i == 0){
			return;
		}
...
db.init(function(){ 
	db.records_by_interval(from, to, function(records){
		...
		db.path_all(from, to, function(paths){
			var fts = paths.slice(0,10).map(function(path){
				var device = {mac:path[0].mac, path:path};
				return new DeviceFeature(device, null, null);
			});
			// console.log(fts);
		})
		...
		console.log(appcps);	
		// console.log()
		var t=d3.select("#canvas");
		myPCP = PCP.init(t, {pos: [100,100], size: [800,600]}, appcps);
	});
});
